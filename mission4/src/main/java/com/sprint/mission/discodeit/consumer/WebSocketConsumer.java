...
public class WebSocketConsumer {
  ...
  @KafkaListener(
      topics = KafkaProduceRequiredEventListener.MESSAGE_CREATED,
      groupId = "#{T(java.net.InetAddress).getLocalHost().getHostName()}"
  )
  public void consume(String payload) {
    log.info(">>>> Kafka 메시지 수신 성공! 그룹ID: {}, payload: {}",
        System.getenv("DISCODEIT_KAFKA_GROUP_ID"), payload);
    try {
      ...
      log.info(">>>> 브로커로 전송 시도: channelId={}", event.channelId());
