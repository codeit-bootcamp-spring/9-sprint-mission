...
public class BasicUserService implements UserService {
  ...
  public UserDto create(UserCreateRequest userCreateRequest,
    ...
    if (userRepository.existsByEmail(email)) {
      ...
      throw new DuplicateUserException().addDetail("email", email);
    }
    if (userRepository.existsByUsername(username)) {
      log.warn("Failed to create user: Username {} already exists", username);
      throw new DuplicateUserException().addDetail("username", username);
    }
  ...
  public UserDto update(UUID userId, UserUpdateRequest userUpdateRequest,
    ...
    if (userRepository.existsByEmail(newEmail)) {
      ...
      throw new DuplicateUserException().addDetail("email", newEmail);
    }
    if (userRepository.existsByUsername(newUsername)) {
      log.warn("Failed to update user: Username {} already exists", newUsername);
      throw new DuplicateUserException().addDetail("username", newUsername);
    }
